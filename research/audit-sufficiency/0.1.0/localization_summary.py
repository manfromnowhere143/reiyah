...
for eps in d['epsilons']:
    ...
    sizes = [a['confirmed_positions'] for a in audits]
    shares = [a['confirmed_positions'] / r['labels'] for r in rows for a in [r['by_epsilon'][k].get('audit_guided')] if a]
    summary['by_epsilon'][k] = {
        ...
        'audit_final_status': dict(Counter(a['final_status'] for a in audits)),
