...
def run_pre():
    ...
    with tempfile.TemporaryDirectory(prefix="compact-mat-controls-") as directory:
        path = Path(directory) / "typed.mat"
        cells = numpy.empty((2, 6), dtype=object)
        cells[0] = audit.TARGET_HEADER
        cells[1] = [sample["Time"], 1.0, 2.0, 0.1, 0.2, numpy.uint8(0)]
        scipy.io.savemat(path, {"BME_HondaCorr": cells})
        test(
            "mat_integer_scalar",
            lambda: require(mat_rows(path)[1][-1] == 0, "integer_scalar"),
        )
        cells[1, 5] = "unknown"
        scipy.io.savemat(path, {"BME_HondaCorr": cells})
        test(
            "mat_text_still_rejected",
            lambda: rejects(lambda: mat_rows(path), "mat_numeric_type"),
        )
        cells[1, 5] = numpy.uint64(2**53 + 1)
        scipy.io.savemat(path, {"BME_HondaCorr": cells})
        test(
            "mat_inexact_integer_rejected",
            lambda: rejects(lambda: mat_rows(path), "mat_numeric_type"),
        )
    return results
