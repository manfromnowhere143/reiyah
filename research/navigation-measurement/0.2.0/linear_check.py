...
def solve(case):
    ...
    return {
        ...
        "minimum_range_mps": [str(lo), str(hi)],
    }
...
def check(case, result):
    ...
    require(
        ...
        and result.get("scope") == "conditional_recorded_model",
        "result_scope",
    )
    ...
    keys(
        ...
        | {
            ...
            "lower_witness",
            "upper_witness",
            "extra_error_allowance_mps",
            "distinguishing_observation",
        },
    )
    require(
        result["minimum_range_mps"] == expected["minimum_range_mps"], "result_extrema"
    )
    low, high = map(F, expected["minimum_range_mps"])
    for kind, target in (("lower", low), ("upper", high)):
        w = result[kind + "_witness"]
        keys(w, ("envelope", "offset_s", "time_s", "minimum_mps"))
        require(w["envelope"] == kind, "witness_kind")
        delta, t, minimum = (
            rational(v, operand=False)
            for v in (w["offset_s"], w["time_s"], w["minimum_mps"])
        )
        require(
    ...
    require(
        result["extra_error_allowance_mps"]
        == {
    ...
    keys(
        ...
        (
            ...
            "lower_world_mps",
            "upper_world_mps",
            "absolute_error_strictly_below_mps",
            "scope",
        ),
    )
    require(
    ...
    t, lo, hi, eps = (
        ...
        for v in (
            ...
            query["lower_world_mps"],
            query["upper_world_mps"],
            query["absolute_error_strictly_below_mps"],
        )
    )
