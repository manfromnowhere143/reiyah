...
from binding import SOURCE_SLICE, bind
...
def main():
    ...
    require(
        actual["source_slice"] == original["source_slice"] == SOURCE_SLICE,
        "source_slice_mapping",
    )
    rows = decode(data)
    require(
    ...
    process = subprocess.run(
        [args.vendor, str(Path(args.sources) / "ncom-sample-tail.bin")],
        capture_output=True,
        text=True,
    )
