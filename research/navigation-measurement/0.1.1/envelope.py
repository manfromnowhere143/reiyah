...
def solve(case):
    ...
    out = {
        ...
        "scope": "conditional_recorded_model",
    }
    if missing:
    ...
    out.update(
        status=status,
        minimum_range_mps=[str(low), str(high)],
        lower_witness={
            "envelope": "lower",
    ...
    if status == "unresolved":
        ...
        out["distinguishing_observation"] = {
            ...
            "lower_world_mps": str(low),
            "upper_world_mps": str(safe_value),
            "absolute_error_strictly_below_mps": str((safe_value - low) / 2),
