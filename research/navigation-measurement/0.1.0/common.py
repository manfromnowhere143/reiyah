...
def parse_case(case):
    ...
    if case["motion"] is not None:
        keys(case["motion"], ("kind", "rate_mps2"))
        require(case["motion"]["kind"] == "global_lipschitz", "motion_kind")
        k = rational(case["motion"]["rate_mps2"])
    ...
    require(type(rows) is list and len(rows) <= 21, "observation_cap")
    points, ids, last = [], set(), None
    for row in rows:
        keys(row, ("id", "time_s", "interval_mps"))
        identifier(row["id"])
