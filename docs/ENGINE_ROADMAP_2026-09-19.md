...
```mermaid
flowchart TD
  Q["Named decision and guarantee"] --> M["Qualified observations and residual uncertainty"]
  M --> E["Exact conditional proof or explicit non-identification"]
  M --> S["Separately qualified statistical audit"]
  E --> C["Same-obligation conventional comparison"]
  S --> C
  C --> R["Genuine revisions and complete measured costs"]
  R --> V{"Consequential repeatable advantage?"}
  V -->|Yes| H["Distinct prospective test and external review"]
  V -->|No| K["Stop the proposed advantage; retain useful kernels"]
```
