"""Result W: does the LLM law replicate on a second benchmark? The red-team of Results T, U.

Results T, U, and V all live on MMLU. The honest objection is overfitting to one benchmark. This
re-runs the core measurements on ARC-Challenge, a different domain, science reasoning rather than
broad knowledge, same seven models, same estimand. If the same-kind-vs-cross-kind coupling, the
conditional coupling, and the over-trusted agreement all reappear, the law is not an MMLU artifact.

Task: harness ARC-Challenge (arc:challenge), 25-shot, per-question correctness and chosen answer
(argmax of per-choice log-probs), joined across models by a hash of the example.
"""
...
def main():
    ...
    n = len(A)
    print("=" * 84)
    print(f"RESULT W - the LLM law on a second benchmark: ARC-Challenge")
    ...
    print("\n  --- replication verdict vs MMLU (T/U) ---")
    print("  MMLU: marginal 1.32 (same 1.52 > cross 1.29), conditional 1.10, jury eff 3.6,")
    print("        agree->correct 64.8%, unanimous->wrong 10.4%.")
    print("  If ARC shows the same pattern (c>1, same>cross, agreement over-trusted), the law holds.")
    print("\n" + "-" * 84)
    print("NON-CLAIMS: public leaderboard outputs on a second benchmark (ARC-Challenge 25-shot);")
