...
Reiyah is independent from sibling repositories. It currently has no assumed remote.
Never import authority, data, code, conclusions, or configuration from another repository.
...
runtime, deployment, physical-control integration, private-data ingestion, publication
machinery, model training or inference, live network dependencies, or operational claims.
Validators may check static repository artifacts but must not implement product behavior.
...
- Do not fetch or cite a source as retained evidence until its bytes, metadata, license or
  access constraints, and digest are recorded locally. A URL alone is not retained evidence.
